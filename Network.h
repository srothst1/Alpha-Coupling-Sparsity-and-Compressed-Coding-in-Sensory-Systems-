...
struct Neuron {
    ...
    bool Espike; // keeps track of whether neuron spike at given time step
};
...
void vspike(Neuron* Network, double tspike, int k) {
    ...
    if (Network[foo].tsp_Ecount >= 999) cout << "We are over the allocated length of tsp_Ecount" << endl;
    Network[foo].Espike = true;
    }
}
